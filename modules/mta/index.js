...
const MTA_STOP_HELP = `Usage: \`?mta-stop D18\`. Use ?mta-stop --list\` to find stops`;
const MTA_HELP = `Usage: \`?mta-stop F\``;
...
module.exports = class MTA extends BaseModule {
  ...
  async postStopData(data) {
    try {
      ...
      this.bot.postMessage(
        data.channel,
        `Couldn't find: ${data.user_text}. ${MTA_STOP_HELP}`
      );
    }
  }
  ...
  createDates(schedule, stopId, direction) {
    ...
    for (let i = 0; i < trains.length; i++) {
      ...
      times.push(
        `${train.delay ? '_*Delayed*_ ' : ''}In ${date.fromNow(
          true
        )} (${date.format('h:mm a')})`
      );
      if (times.length === 3) {
  ...
  postMsg(data, line) {
    ...
    if (line.status !== 'GOOD SERVICE') {
      msg = `${line.status}\nFor more info: http://www.mta.info/status/subway/${
        line.name
      }`;
    }
